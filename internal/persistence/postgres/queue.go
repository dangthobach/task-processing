...
func (s *Store) PendingOutbox(ctx context.Context, limit int) ([]OutboxMessage, error) {
	...
	rows, err := s.Pool.Query(ctx, `SELECT o.id,COALESCE(b.backend_type,'POSTGRES'),r.id,r.project_id,r.queue_id,r.priority,r.available_at,r.payload FROM outbox_events o JOIN job_runs r ON r.id=o.aggregate_id JOIN queues q ON q.id=r.queue_id LEFT JOIN queue_backends b ON b.id=q.backend_id WHERE o.event_type='job.enqueue' AND o.published_at IS NULL AND o.available_at<=now() ORDER BY o.available_at,o.id LIMIT $1`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&item.ID, &item.BackendType, &item.RunID, &item.ProjectID, &item.QueueID, &item.Priority, &item.AvailableAt, &item.Payload); err != nil {
			return nil, err
		}
...
func (s *Store) MarkOutboxPublished(ctx context.Context, id uuid.UUID) error {
	_, err := s.Pool.Exec(ctx, "UPDATE outbox_events SET published_at=now(),attempts=attempts+1,last_error=NULL WHERE id=$1 AND published_at IS NULL", id)
	return err
}
func (s *Store) RecordOutboxFailure(ctx context.Context, id uuid.UUID, cause error) error {
	_, err := s.Pool.Exec(ctx, "UPDATE outbox_events SET attempts=attempts+1,last_error=$2,available_at=now()+(LEAST(attempts,8) * interval '1 second') WHERE id=$1 AND published_at IS NULL", id, truncate(cause.Error(), 1000))
	return err
}

// EnqueueRuns is the PostgreSQL implementation of QueueBackend.EnqueueBatch.
// It is transactional: either every accepted run becomes claimable or none do.
func (s *Store) EnqueueRuns(ctx context.Context, runs []uuid.UUID) error {
	if len(runs) == 0 {
		return nil
	}
	...
	for _, runID := range runs {
		tag, err := tx.Exec(ctx, "UPDATE job_runs SET status='QUEUED',updated_at=now() WHERE id=$1 AND status='ENQUEUE_PENDING'", runID)
		if err != nil {
			return err
		}
		if tag.RowsAffected() == 0 {
			var status string
			if err = tx.QueryRow(ctx, "SELECT status FROM job_runs WHERE id=$1", runID).Scan(&status); err != nil {
				return err
			}
			if status != "QUEUED" {
				return fmt.Errorf("run %s is not enqueue-pending", runID)
			}
		}
	}
...
func (s *Store) DispatchOutbox(ctx context.Context, limit int) (int, error) {
	...
	defer tx.Rollback(ctx)
	rows, err := tx.Query(ctx, `SELECT id,aggregate_id FROM outbox_events WHERE published_at IS NULL AND available_at<=now() ORDER BY available_at,id FOR UPDATE SKIP LOCKED LIMIT $1`, limit)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
	...
	for _, id := range runIDs {
		if _, err = tx.Exec(ctx, "UPDATE job_runs SET status='QUEUED',updated_at=now() WHERE id=$1 AND status='ENQUEUE_PENDING'", id); err != nil {
			return 0, err
		}
	}
	for _, id := range ids {
...
func (s *Store) PromoteRetries(ctx context.Context) (int64, error) {
	tag, err := s.Pool.Exec(ctx, "UPDATE job_runs SET status='QUEUED',updated_at=now() WHERE status='RETRY_WAIT' AND available_at<=now()")
	return tag.RowsAffected(), err
}
func (s *Store) RecoverExpiredLeases(ctx context.Context) (int64, error) {
	tag, err := s.Pool.Exec(ctx, "UPDATE job_runs SET status='QUEUED',lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE status IN ('RESERVED','RUNNING') AND lease_expires_at<now()")
	return tag.RowsAffected(), err
}
func (s *Store) ExtendLease(ctx context.Context, runID uuid.UUID, owner string, lease time.Duration) error {
	tag, err := s.Pool.Exec(ctx, "UPDATE job_runs SET lease_expires_at=now()+($3 * interval '1 millisecond'),updated_at=now() WHERE id=$1 AND status='RUNNING' AND lease_owner=$2", runID, owner, lease.Milliseconds())
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("cannot extend lease for run %s", runID)
	}
	return nil
}
...
func (s *Store) Claim(ctx context.Context, workerID uuid.UUID, owner string, limit int, lease time.Duration) ([]job.Run, error) {
	...
	for rows.Next() {
		var r job.Run
		var policy []byte
		if err = rows.Scan(&r.ID, &r.ProjectID, &r.DefinitionID, &r.QueueID, &r.IdempotencyKey, &r.Priority, &r.Payload, &r.FunctionVersion, &policy); err != nil {
			return nil, err
		}
...
func (s *Store) StartAttempt(ctx context.Context, runID, workerID uuid.UUID, owner string, lease time.Duration) (job.Execution, error) {
	tx, err := s.Pool.Begin(ctx)
	if err != nil {
	...
	if err = tx.QueryRow(ctx, `SELECT t.id,r.project_id,r.queue_id,r.job_definition_id,q.max_concurrency,fd.max_concurrency,p.max_concurrency,t.max_concurrency FROM job_runs r JOIN projects p ON p.id=r.project_id JOIN tenants t ON t.id=p.tenant_id JOIN queues q ON q.id=r.queue_id JOIN job_definitions jd ON jd.id=r.job_definition_id JOIN function_definitions fd ON fd.id=jd.function_id WHERE r.id=$1 AND r.status='RESERVED' AND r.lease_owner=$2 AND t.deleted_at IS NULL AND p.deleted_at IS NULL AND q.deleted_at IS NULL AND jd.deleted_at IS NULL AND fd.deleted_at IS NULL FOR UPDATE OF r`, runID, owner).Scan(&tenantID, &r.ProjectID, &r.QueueID, &r.DefinitionID, &queueLimit, &functionLimit, &projectLimit, &tenantLimit); err != nil {
		return job.Execution{}, err
	}
	...
	for _, scope := range []string{"tenant:" + tenantID.String(), "project:" + r.ProjectID.String(), "queue:" + r.QueueID.String(), "function:" + r.DefinitionID.String()} {
	...
	checks := []struct {
		...
		{"SELECT count(*) FROM job_runs WHERE status='RUNNING' AND lease_expires_at>now() AND job_definition_id=$1", r.DefinitionID, functionLimit},
	}
	...
	var keyRef string
	err = tx.QueryRow(ctx, `UPDATE job_runs r SET status='RUNNING',started_at=COALESCE(started_at,now()),lease_expires_at=now()+($3 * interval '1 millisecond'),updated_at=now() FROM job_definitions jd JOIN function_definitions fd ON fd.id=jd.function_id WHERE r.id=$1 AND r.job_definition_id=jd.id AND r.status='RESERVED' AND r.lease_owner=$2 AND jd.deleted_at IS NULL AND fd.deleted_at IS NULL RETURNING r.id,r.function_version,fd.function_key,r.idempotency_key,r.payload,r.payload_ciphertext,COALESCE(r.encryption_key_ref,''),r.policy_snapshot`, runID, owner, lease.Milliseconds()).Scan(&r.ID, &r.FunctionVersion, &r.FunctionKey, &r.IdempotencyKey, &r.Payload, &ciphertext, &keyRef, &policy)
	if err != nil {
		return job.Execution{}, err
	}
	if err = json.Unmarshal(policy, &r.Policy); err != nil {
	...
	return job.Execution{RunID: runID, AttemptID: attemptID, TenantID: tenantID, ProjectID: r.ProjectID, QueueID: r.QueueID, DefinitionID: r.DefinitionID, FunctionKey: r.FunctionKey, FunctionVersion: r.FunctionVersion, IdempotencyKey: r.IdempotencyKey, Payload: r.Payload, Policy: r.Policy, Attempt: n}, nil
}
...
func (s *Store) ReleaseReservation(ctx context.Context, runID uuid.UUID, owner string) error {
	_, err := s.Pool.Exec(ctx, "UPDATE job_runs SET status='QUEUED',lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE id=$1 AND status='RESERVED' AND lease_owner=$2", runID, owner)
	return err
}
func (s *Store) CompleteSuccess(ctx context.Context, ex job.Execution, owner string) error {
	...
	tag, err := tx.Exec(ctx, "UPDATE job_runs SET status='SUCCEEDED',finished_at=now(),lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE id=$1 AND status='RUNNING' AND lease_owner=$2", ex.RunID, owner)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("run %s no longer leased by worker", ex.RunID)
	}
...
func (s *Store) CompleteFailure(ctx context.Context, ex job.Execution, owner string, class *job.ClassifiedError, policy job.RetryPolicy, now time.Time) error {
	...
	tag, err := tx.Exec(ctx, "UPDATE job_runs SET status=$3,available_at=$4,finished_at=CASE WHEN $3='DEAD_LETTER' THEN now() ELSE NULL END,lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE id=$1 AND status='RUNNING' AND lease_owner=$2", ex.RunID, owner, status, available)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
...
func (s *Store) ReportProgress(ctx context.Context, ex *job.Execution, owner string, percent int, message string) error {
	...
	defer tx.Rollback(ctx)
	tag, err := tx.Exec(ctx, `UPDATE job_attempts a SET progress_pct=$3,progress_message=$4,updated_at=now() FROM job_runs r WHERE a.id=$1 AND a.job_run_id=r.id AND r.id=$2 AND r.status='RUNNING' AND r.lease_owner=$5`, ex.AttemptID, ex.RunID, percent, truncate(message, 500), owner)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("attempt %s is no longer running", ex.AttemptID)
	}
